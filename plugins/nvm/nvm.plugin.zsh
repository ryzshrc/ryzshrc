...
function _ryz_nvm_setup_completion {
  ...
  unfunction _ryz_nvm_setup_completion
}

function _ryz_nvm_setup_autoload {
  if ! zstyle -t ':ryz:plugins:nvm' autoload; then
    unfunction _ryz_nvm_setup_autoload
    return
  fi
  ...
  function load-nvmrc {
    ...
    local nvm_silent=""
    zstyle -t ':ryz:plugins:nvm' silent-autoload && nvm_silent="--silent"
...
if zstyle -t ':ryz:plugins:nvm' lazy; then
  # Call nvm when first using nvm, node, npm, pnpm, yarn, corepack or other commands in lazy-cmd
  zstyle -a ':ryz:plugins:nvm' lazy-cmd nvm_lazy_cmd
  nvm_lazy_cmd=(_ryz_nvm_load nvm node npm npx pnpm pnpx yarn corepack $nvm_lazy_cmd) # default values
  eval "
    function $nvm_lazy_cmd {
      ...
      _ryz_nvm_setup_completion
      _ryz_nvm_setup_autoload
      if [[ \"\$0\" != _ryz_nvm_load ]]; then
        \"\$0\" \"\$@\"
      fi
    }
  "
