...
_source-from-ryz-settings() {
  local _chruby_path _chruby_auto

  zstyle -s :ryz:plugins:chruby path _chruby_path || return 1
  zstyle -s :ryz:plugins:chruby auto _chruby_auto || return 1
...
# Load chruby
if _source-from-ryz-settings; then
  _load-chruby-dirs
...
unfunction _source-from-homebrew _source-from-ryz-settings _load-chruby-dirs
