alias-finder() {
  ...
  zstyle -t ':ryz:plugins:alias-finder' longer && longer=true
  zstyle -t ':ryz:plugins:alias-finder' exact && exact=true
  zstyle -t ':ryz:plugins:alias-finder' cheaper && cheaper=true
...
preexec_alias-finder() {
  # TODO: Remove backward compatibility (other than zstyle form)
  zstyle -t ':ryz:plugins:alias-finder' autoload && alias-finder $1 || if [[ $ZSH_ALIAS_FINDER_AUTOMATIC = true ]]; then
    alias-finder $1
  fi
}
