...
# example, to change the path to the RyZshrc repository:
...
# a) if $ZDOTDIR is supplied and not $HOME: $ZDOTDIR/ryzshrc
# b) otherwise, $HOME/.ry-zsh-rc
if [ -n "$ZDOTDIR" ] && [ "$ZDOTDIR" != "$HOME" ]; then
  ZSH="${ZSH:-$ZDOTDIR/ryzshrc}"
fi
ZSH="${ZSH:-$HOME/.ry-zsh-rc}"
...
setup_ryzshrc() {
  ...
  echo "${FMT_BLUE}Cloning RyZshrc...${FMT_RESET}"
  ...
  && git config ry-zsh-rc.remote origin \
  && git config ry-zsh-rc.branch "$BRANCH" \
  ...
  && git checkout -b "$BRANCH" "origin/$BRANCH" || {
    ...
    fmt_error "git clone of ry-zsh-rc repo failed"
    exit 1
  }
...
setup_zshrc() {
  # Keep most recent old .zshrc at .zshrc.pre-ry-zsh-rc, and older ones
  # with datestamp of installation that moved them aside, so we never actually
  # destroy a user's original zshrc
  echo "${FMT_BLUE}Looking for an existing zsh config...${FMT_RESET}"

  # Must use this exact name so uninstall.sh can find it
  OLD_ZSHRC="$zdot/.zshrc.pre-ry-zsh-rc"
  if [ -f "$zdot/.zshrc" ] || [ -h "$zdot/.zshrc" ]; then
    ...
    if [ -e "$OLD_ZSHRC" ]; then
      ...
      echo "${FMT_YELLOW}Found old .zshrc.pre-ry-zsh-rc." \
  ...
  echo "${FMT_GREEN}Using the RyZshrc template file and adding it to $zdot/.zshrc.${FMT_RESET}"
...
EOF
  ...
  if [ -n "$SHELL" ]; then
    echo "$SHELL" > "$zdot/.shell.pre-ry-zsh-rc"
  else
    grep "^$USER:" /etc/passwd | awk -F: '{print $7}' > "$zdot/.shell.pre-ry-zsh-rc"
  fi
...
2. Install RyZshrc to a directory that doesn't exist yet:
   $(fmt_code "ZSH=path/to/new/ryzshrc/folder sh install.sh")
