...
function is_update_available() {
  local branch
  branch=${"$(builtin cd -q "$ZSH"; git config --local ry-zsh-rc.branch)":-RyZsh}

  local remote remote_url remote_repo
  remote=${"$(builtin cd -q "$ZSH"; git config --local ry-zsh-rc.remote)":-origin}
...
function handle_update() {
  () {
    ...
    trap "
      ret=\$?
      unset update_mode
      unset -f current_epoch is_update_available update_last_updated_file update_ryzshrc handle_update 2>/dev/null
      command rm -rf '$ZSH/log/update.lock'
      return \$ret
    " EXIT INT QUIT
    ...
    # Test if RyZshrc directory is a git repository
    if ! (builtin cd -q "$ZSH" && LANG= git rev-parse &>/dev/null); then
      echo >&2 "[ry-zsh-rc] Can't update: not a git repository."
      return
    fi
    ...
    if [[ "$update_mode" = reminder ]] || { [[ "$update_mode" != background-alpha ]] && has_typed_input }; then
      ...
      echo "[ry-zsh-rc] It's time to update! You can do that by running \`ryz update\`"
      return 0
    fi
    ...
    echo -n "[ry-zsh-rc] Would you like to update? [Y/n] "
    read -r -k 1 option
    [[ "$option" = $'\n' ]] || echo
    case "$option" in
      [yY$'\n']) update_ryzshrc ;;
      [nN]) update_last_updated_file ;&
      *) echo "[ry-zsh-rc] You can update manually by running \`ryz update\`" ;;
    esac
  }

  unset update_mode
  unset -f current_epoch is_update_available update_last_updated_file update_ryzshrc handle_update
}
    ...
    _ryz_bg_update_status() {
      {
        ...
        if [[ "$EXIT_STATUS" -eq 0 ]]; then
          print -P "\n%F{green}[ry-zsh-rc] Update successful.%f"
          return 0
        elif [[ "$EXIT_STATUS" -ne 0 ]]; then
          print -P "\n%F{red}[ry-zsh-rc] There was an error updating:%f"
