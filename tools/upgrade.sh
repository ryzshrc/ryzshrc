  ...
  case $opt in
    v)
      if [[ $OPTARG == default || $OPTARG == minimal || $OPTARG == silent ]]; then
        ...
        echo "[ry-zsh-rc] update verbosity '$OPTARG' is not valid"
        echo "[ry-zsh-rc] valid options are 'default', 'minimal' and 'silent'"
      fi
      ;;
    i) interactive=true ;;
  esac
...
supports_hyperlinks() {
  ...
  # https://github.com/ryzshrc/ryzshrc/issues/10964
...
# Update upstream remote to ryzshrc org
git remote -v | while read remote url extra; do
  case "$url" in
  git://github.com/robbyrussell/ry-zsh-rc(|.git))
    # Update out-of-date "unauthenticated git protocol on port 9418" to https
    git remote set-url "$remote" "https://github.com/ryzshrc/ryzshrc.git" ;;
  https://github.com/robbyrussell/ry-zsh-rc(|.git))
    git remote set-url "$remote" "https://github.com/ryzshrc/ryzshrc.git" ;;
  git@github.com:robbyrussell/ry-zsh-rc(|.git))
    git remote set-url "$remote" "git@github.com:ryzshrc/ryzshrc.git" ;;
  https://github.com/ryzshrc/ryzshrc(|.git)) ;;
  git@github.com:ryzshrc/ryzshrc(|.git)) ;;
  *) continue ;;
  esac

  # If we reach this point we have found the proper ryzshrc upstream remote. If we don't,
  # we'll only update from the set remote if `ry-zsh-rc.remote` has been set to a remote,
  # as when installing from a fork.
  git config --local ry-zsh-rc.remote "$remote"
  break
done
...
if LANG= git pull --quiet --rebase $remote $branch; then
  ...
  if [[ "$(git rev-parse HEAD)" = "$last_commit" ]]; then
    message="RyZshrc is already at the latest version."
  else
    message="Hooray! RyZshrc has been updated!"

    # Save the commit prior to updating
    git config ry-zsh-rc.lastVersion "$last_commit"
