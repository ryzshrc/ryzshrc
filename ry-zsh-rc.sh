...
# Protect against non-zsh execution of RyZshrc (use POSIX syntax here)
[ -n "$ZSH_VERSION" ] || {
  ryz_ptree() {
...

  {
    ...
    printf "$(ryz_f 1 31)Error:$(ryz_f 22) RyZshrc can't be loaded from: $(ryz_f 1)${shell}$(ryz_f 22). "
...
# https://github.com/ryzshrc/ryzshrc/issues/11686
[[ "$(emulate)" = zsh ]] || {
  printf "$(ryz_f 1 31)Error:$(ryz_f 22) RyZshrc can't be loaded in \`$(emulate)\` emulation mode.$(ryz_f 0)\n" >&2
  return 1
}
...
if [[ ! -w "$ZSH_CACHE_DIR" ]]; then
  ZSH_CACHE_DIR="${XDG_CACHE_HOME:-$HOME/.cache}/ry-zsh-rc"
fi
...
for plugin ($plugins); do
  if is_plugin "$ZSH_CUSTOM" "$plugin"; then
    ...
    echo "[ry-zsh-rc] plugin '$plugin' not found"
  fi
done
...
mkdir -p "${ZDOTDIR:-$HOME}/.RyZsh"

# Reload the zcompdump configuration if necessary.
...
if (( $zcompdump_refresh )) \
  ...
  # See https://github.com/ryzshrc/ryzshrc/commit/dd1a7269#commitcomment-39003489
...
# Load all of the lib files in ~/ry-zsh-rc/lib that end in .zsh
...
if [[ -n "$ZSH_THEME" ]]; then
  if is_theme "$ZSH_CUSTOM" "$ZSH_THEME"; then
    ...
    echo "[ry-zsh-rc] theme '$ZSH_THEME' not found"
  fi
fi
