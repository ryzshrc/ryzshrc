...
function ryz_termsupport_precmd {
  [[ "${DISABLE_AUTO_TITLE:-}" != true ]] || return
  title "$ZSH_THEME_TERM_TAB_TITLE_IDLE" "$ZSH_THEME_TERM_TITLE_IDLE"
}

# Runs before executing the command
function ryz_termsupport_preexec {
  [[ "${DISABLE_AUTO_TITLE:-}" != true ]] || return
...
if [[ -z "$INSIDE_EMACS" || "$INSIDE_EMACS" = vterm ]]; then
  add-zsh-hook precmd ryz_termsupport_precmd
  add-zsh-hook preexec ryz_termsupport_preexec
fi
...
function ryz_termsupport_cwd {
  # Percent-encode the host and path names.
  local URL_HOST URL_PATH
  URL_HOST="$(ryz_urlencode -P $HOST)" || return 1
  URL_PATH="$(ryz_urlencode -P $PWD)" || return 1
...
add-zsh-hook precmd ryz_termsupport_cwd
