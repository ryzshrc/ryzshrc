...
function _ryz_git_prompt_info() {
  ...
  if ! __git_prompt_git rev-parse --git-dir &> /dev/null \
    || [[ "$(__git_prompt_git config --get ry-zsh-rc.hide-info 2>/dev/null)" == 1 ]]; then
    return 0
  fi
...
# - https://github.com/ryzshrc/ryzshrc/issues/12331
# - https://github.com/ryzshrc/ryzshrc/issues/12360
...
function parse_git_dirty() {
  ...
  local -a FLAGS
  FLAGS=('--porcelain')
  if [[ "$(__git_prompt_git config --get ry-zsh-rc.hide-dirty)" != "1" ]]; then
...
function _ryz_git_prompt_status() {
  [[ "$(__git_prompt_git config --get ry-zsh-rc.hide-status 2>/dev/null)" = 1 ]] && return
