...
function _ryz {
  ...
  cmds=(
    ...
    'pr:Manage RyZshrc Pull Requests'
    'reload:Reload the current zsh session'
    'theme:Manage themes'
    'update:Update RyZshrc'
...
Available commands:
  ...
  pr     <command>    Manage RyZshrc Pull Requests
  reload              Reload the current zsh session
...
Available commands:
  ...
  list             List all available RyZshrc plugins
...
function _ryz::pr::clean {
  (
    ...
    branches="$(command git for-each-ref --sort=-committerdate --color --format="$fmt" "refs/heads/ryzshrc/pull-*")"
    ...
    _ryz::log info "removing all RyZshrc Pull Request branches..."
    command git branch --list 'ryzshrc/pull-*' | while read branch; do
...
function _ryz::pr::test {
  ...
  (
    ...
    # Get the ryzshrc git remote
    command git remote -v | while read remote url _; do
      case "$url" in
      https://github.com/ryzshrc/ryzshrc(|.git)) found=1; break ;;
      git@github.com:ryzshrc/ryzshrc(|.git)) found=1; break ;;
      esac
    done

    (( $found )) || {
      _ryz::log error "could not found the ryzshrc git remote. Aborting..."
      return 1
    }

    # Fetch pull request head
    _ryz::log info "fetching PR #$1 to ryzshrc/pull-$1..."
    command git fetch -f "$remote" refs/pull/$1/head:ryzshrc/pull-$1 || {
...
Available commands:
  ...
  list            List all available RyZshrc themes
