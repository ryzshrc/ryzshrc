...
if [[ -n "$ZSH_THEME_RANDOM_BLACKLIST" ]]; then
  echo '[ry-zsh-rc] ZSH_THEME_RANDOM_BLACKLIST is deprecated. Use `ZSH_THEME_RANDOM_IGNORED` instead.'
...
if [[ -f "$ZSH_CUSTOM/$RANDOM_THEME.zsh-theme" ]]; then
  ...
  echo "[ry-zsh-rc] Random theme '${RANDOM_THEME}' not found"
  return 1
fi

if [[ "$ZSH_THEME_RANDOM_QUIET" != true ]]; then
  echo "[ry-zsh-rc] Random theme '${RANDOM_THEME}' loaded"
fi
